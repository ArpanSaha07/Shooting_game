...
for i in range(1, 4):
    ...
    if i < 3:
        for j in range(1, 4):
            target_images[i - 1].append(pygame.transform.scale(pygame.image.load(f'assets/targets/{i}/{j}.png'), (120 -, 100)))
...
def draw_gun():
    ...
    if mouse_pos[0] != gun_point[0]:            # to avoid zero division error
        gradient = (mouse_pos[1] - gun_point[1])/ (mouse_pos[0] - gun_point[0])
    else:
        gradient = -10000000
    angle = math.atan(gradient)
    rotation = math.degrees(angle)

    if mouse_pos[0] < WIDTH/2:      # if mouse pointer is in the left side
